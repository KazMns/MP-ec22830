...
public class RunElection extends Dropdown{
    ...
    private List<Candidate> defaultSet = getCandidateList(getCandidateArray());
    ...
    public RunElection() {
        ...
        runElectionButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (e.getSource()==runElectionButton){
                    int getValue = Integer.parseInt(textField1.getText());

                    String[] buttons = { "Random 1-election", "Random 2-election", "Close"};
                    ...
                    s = Integer.parseInt(JOptionPane.showInputDialog( "Enter Seeds"));

                    if (returnValue==0){
                        randomElection(all, all, getValue);
                    }

                    else if(returnValue ==1){
                        random2Election(all, all);
                    }
        ...
        addCandidateButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String[] buttons = { "Add Specific Candidate", "Create a New Candidate"};
                int returnValue = JOptionPane.showOptionDialog(null, "Choose Type", "Choose Type",
                        JOptionPane.OK_CANCEL_OPTION, 0, null, buttons, buttons);
    ...
    public void printCandidates(){
        for (int i=0; i<candidates.size(); i++){
            String s = candidates.get(i).getName()+": "+candidates.get(i).getSlogan();
            textArea1.append(s+"\n");
        }
    }

    public void addRandomCandidates(int nb){
        for (int i=0; i<nb; i++){

            int x = random.nextInt(546);

            candidates.add(defaultSet.get(x));
        }
    }

    public void randomElection(Candidate[] array1, Candidate[] array2, int i){
        if (candidates.size()!=0){
            int difference = i - candidates.size();
            addRandomCandidates(difference);
        }

        printCandidates();
    }

    public void random2Election(Candidate[] array1, Candidate[] array2){

    }

    public static Candidate getByUsername(String name, Candidate[] ca) {
        for (int i=1; i<ca.length; i++){
            if (ca[i].un.equals(name)){
                return ca[i];
            }
        }
        return null;
    ...
    public Candidate[] getCandidateArray() {
        ...
        ca[1] = new Candidate_ah21407(); ca[1].un = "ah21407";
    ...
    public List<Candidate> getCandidateList(Candidate[] ca){
        return new ArrayList<>(Arrays.asList(ca));
    }
}
