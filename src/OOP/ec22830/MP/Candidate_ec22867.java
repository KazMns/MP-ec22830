package OOP.ec22830.MP;// File Candidate_ec22867.java
//
...
class Candidate_ec22867 extends Candidate {
    ...
    private static void printCandidates(Candidate[] candidates, int counter) {
    ...
    private static void runElection(Candidate[] votes, int counter, int howManyTimes, Candidate voteCounter, Candidate[] allCandidates){
        ...
        Candidate[] winningCandidates = new Candidate[howManyTimes * counter];
        ...
        for (int i = 0; i < howManyTimes; i++) {
            for (int j = 0; j < counter; j++) {
                try {
                    ...
                    winningCandidates[(counter * i) + j] = new Candidate_ec221156();
                }
            }
        }
    ...
    public String getSlogan() {
        return "Everthing happens for a reason";
    }
    ...
    public Candidate vote(Candidate[] candidates) {
        ...
        return new Candidate_ec22867();
        
        // Search for self on the list of candidates.
        for (Candidate c : candidates)
            if (c.getName().equals("Sana"))
                return c;
