...
class A3 {
    ...
    public static void main(String[] args) {
        ...
        p("This program was written by ");
        int count = 0; 
        for (Candidate c : all) if (proper(c)) count++;
        pr(count + " people: ");
        for (Candidate c : all) if (proper(c)) p(c.un+" ");
        pr("");
        ...
        do {
          ...
          switch (choice) {
            ...
            case 'b': showErrors = !showErrors; break;
                  
            case 'c': randomElection(all, all); break;
                  
            case 'd': random2Election(all, all); break;
    ...
    private static void random2Election(Candidate[] possibleCandidates, Citizen[] voters) {
        
        final Citizen trustedCounter = new Candidate_eey577();
        
        int number = getInt("How many candidates?");
        Candidate[] candidates = randomCandidates(number, possibleCandidates);
        
        do {
        Candidate[] votes = getVotes(candidates, voters);
        ...
        Candidate[] winners = getWinners(votes, voters);
